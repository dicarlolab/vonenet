...
def generate_gabor_param(features, seed=0, rand_flag=False, sf_corr=0, sf_max=9,
                         sf_min=0):
    ...
    if rand_flag:
        ...
        sf_dist = np.array([1, 1, 1, 1, 1, 1, 1, 1, 1])
        ...
        # DeValois 1982b
        sf_bins = np.array([0.5, 0.7, 1.0, 1.4, 2.0, 2.8, 4.0, 5.6, 8, 11.2])
        sf_dist = np.array([4, 4, 8, 25, 32, 26, 28, 12, 8])
    ...
    if rand_flag:
        ...
        nx = np.interp(samps_cdf[:, 0], np.hstack(([0], nx_dist.cumsum())),
                       np.log10(nx_bins))
        nx = 10 ** nx
        ...
        for samp_ind, nx_samp in enumerate(nx):
            ...
            ny[samp_ind] = np.interp(ny_samp[samp_ind], np.hstack(
                ([0], ny_dist_marg[bin_id, :].cumsum())),
                                     np.log10(ny_bins))
        ny = 10 ** ny

        sf = np.interp(samps_cdf[:, 1], np.hstack(([0], sf_dist.cumsum())),
                       np.log2(sf_bins))
        sf = 2 ** sf
